...
def new_pos(current, instruction, facing=None):
    ...
    # we want to find the first location we have already been at
    # that includes locations we are passing not only the end location
    for i in range(1, distance):
        loc = (cx+xsign*i,cy+ysign*i)
        VISITED[loc] += 1
        if VISITED[loc] > 1:
            return loc, facing, True
    return (cx+xsign*distance,cy+ysign*distance), facing, False
...
for instruction in instructions.split(","):
    ...
    current, facing, stop = new_pos(current, instruction, facing)
    if stop:
        break
